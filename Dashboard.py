...
def main():
    ...
    if is_authenticated:
        ...
        charge_equeteur=data_rep["ID_Enquêteur"].value_counts()
        charge_controleur=data_rep["ID_Contrôleur"].value_counts()
        charge_superviseur=data_rep["ID_Superviseur"].value_counts()
        ...
        with tabs[0]:
            ...
            with col[0] :
                type_questionnaire=st.selectbox("Type de Questionnaire", options=data_to_plot["Type_Quest"].unique())
                if type_questionnaire=="Enseignant":
                    ...
                    fond_color= "blues"
                data_to_plot_cart=data_to_plot[data_to_plot["Type_Quest"]==type_questionnaire]
        ...
        with tabs[1]:
            ...
            with col1[1]:
                ...
                with subcol1[0]:
                    ...
                    make_bar(data_superviz,var="Résultat", height=500,titre=traduire_texte("Qualité des questionaires",lang),color=5)
                with subcol1[1]:
                    ...
                    data_without_coord=data_superviz[(data_superviz["Longitude"].isna()) | (data_superviz["Latitude"].isna())]
            ...
            tcol=st.columns(2)
            with tcol[0]:
                superviz_for_heat_map=st.multiselect(traduire_texte("Sélectionner le (s) superviseur (s)",lang),data_to_plot["ID_Supervi"].unique(),default=data_to_plot["ID_Supervi"].unique())
            with tcol[1]:
                enq_type=st.multiselect(traduire_texte("Sélectionner un type de questionnaire",lang),data_to_plot["Type_Quest"].unique(), default=data_to_plot["Type_Quest"].unique()[1])
            data_superviz_heat_map=data_to_plot[(data_to_plot["ID_Supervi"].isin(superviz_for_heat_map)) & (data_to_plot["Type_Quest"].isin(enq_type))]
            cross_enq=pd.crosstab(data_superviz_heat_map["ID_Enquêt"],data_superviz_heat_map["Date"])
            make_st_heatmap_echat(cross_enq,title=traduire_texte("Charge de travail accomplie par enquêteur",lang)) if data_superviz_heat_map.shape[0]>0 else None
